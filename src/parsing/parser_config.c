...
static int	handle_empty_line(t_config_parsing_state *parse_state)
{
...
static int	validate_line_type(t_line_type line_type,
		...
		t_config_parsing_state *parse_state, t_line_type *line_type_out)
{
	(void)game;
	if (!*trimmed)
		return (handle_empty_line(parse_state));
	...
	*line_type_out = get_line_type(trimmed);
	return (validate_line_type(*line_type_out, parse_state));
}
...
static int	validate_config_completeness(t_config_parsing_state *parse_state)
{
...
int	process_configurations(char **lines, int map_start_line, t_game *game)
{
	...
	return (validate_config_completeness(&parse_state));
}
